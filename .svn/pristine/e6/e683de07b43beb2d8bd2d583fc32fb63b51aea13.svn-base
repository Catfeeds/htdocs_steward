...
Route::domain('open-api.{env}.ebsig.com')->middleware(['api.open'])->group(function() {
    Route::post('goods/add/batch', 'Api\Open\GoodsController@batchAdd'); //批量新增商品
    Route::post('goods/price/batch', 'Api\Open\GoodsController@batchPrice'); //批量修改门店商品价格
    Route::post('goods/store/batch', 'Api\Open\GoodsController@batchStore'); //批量修改门店商品库存
    Route::post('order/get', 'Api\Open\OrderController@get'); //获取订单
});
