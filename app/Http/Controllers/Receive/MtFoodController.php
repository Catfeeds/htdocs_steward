...
class MtFoodController extends Controller
{
    ...
    public function index(Request $request, $type)
    {
        ...
        if ($type != 'create' && $type != 'mall_status') {
        ...
        switch (true) {
            ...
            //门店状态变更回调
            case $type == 'mall_status':
                $res = $this->mallStatePush($content);
                break;
    ...
    public function create($data,$app) {
        ...
        error_log('extras');
        error_log(var_export($fee_data,true));
     ...
     * @return array
     */
    private function mallStatePush($args_data)
    {

        $result = StAppMall::where(['mall_code' => $args_data['app_poi_code'],'app_id' => 100003])->first();

        if (!$result) {
            return ['code' => 404, 'message' => '门店不存在'];
        }

        if ($args_data['poi_status'] == 121) {
            StAppMall::where(['mall_code' => $args_data['app_poi_code'],'app_id' => 100003])
                            ->update(['status'=>1,'updated_at'=>$args_data['operate_time'],'creator'=>$args_data['operate_user']]);
        }elseif ($args_data['poi_status'] == 120){
            StAppMall::where(['mall_code' => $args_data['app_poi_code'],'app_id' => 100003])
                            ->update(['status'=>0,'updated_at'=>$args_data['operate_time'],'creator'=>$args_data['operate_user']]);
        }elseif ($args_data['poi_status'] == 18){
            StAppMall::where(['mall_code' => $args_data['app_poi_code'],'app_id' => 100003])
                            ->update(['online_status'=>1,'updated_at'=>$args_data['operate_time'],'creator'=>$args_data['operate_user']]);
        }elseif ($args_data['poi_status'] == 19){
            StAppMall::where(['mall_code' => $args_data['app_poi_code'],'app_id' => 100003])
                            ->update(['online_status'=>2,'updated_at'=>$args_data['operate_time'],'creator'=>$args_data['operate_user']]);
        }
