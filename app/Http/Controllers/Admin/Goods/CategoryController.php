...
class CategoryController extends Controller
{
    ...
    public function edit(Request $request) {
        ...
        //应用同步
        $st_app_category = StAppCategory::select('st_app_category.o_category_id','st_app_category.app_id','st_app_category.mall_id',
                                                'st_app_mall.o_mall_id','st_app_category.category_name','st_app_category.level')
    ...
    public function sort(Request $request)
    {
        ...
        $st_app_category = StAppCategory::select('st_app_category.app_id','st_app_category.mall_id','st_app_category.category_id'
                                    ,'st_app_category.o_category_id','st_app_category.p_id')
                                ->leftJoin('st_category','st_category.id','=','st_app_category.category_id')
                                ->whereIn('st_app_category.category_id',$category_ids)
                                ->orderBy('st_category.sort','ASC')
                                ->get();
        ...
        if( !$st_app_category -> isEmpty()){
            ...
            foreach ( $st_app_category as $item){
                $category_datas[$item -> app_id][] = $item ;
            }

            foreach ( $category_datas as $key => $datas ) {

                $args_data = [];
                foreach ($datas as $data) {
                    $args_data[$data->mall_id]['o_category_id'][] = $data->o_category_id;
                    $args_data[$data->mall_id]['level'] = $level;
                    $args_data[$data->mall_id]['pid'] = $data->p_id;
                    $args_data[$data->mall_id]['mall_id'] = $data->mall_id;
                }

                foreach ( $args_data as $args){

                    $res = Wm::send( $key .'.goods.set_sort_category', $args);
                    if( $res['code'] != 200 ){
    ...
    public function sync(Request $request){
        ...
        if( !$st_app->isEmpty()){
            ...
            foreach ( $st_app as $app ){
                ...
                $st_app_mall = StAppMall::where([['online_status',1],['app_id',$app -> id ]])->get();
                            ...
                            foreach ( $st_category as $category ){
                                ...
                                if( !empty($category['children'])){
                                    ...
                                    foreach ( $category['children'] as $child ){
                                        ...
                                        $st_app_category = StAppCategory::where([['app_id',$app -> id ],['mall_id',$mall->mall_id],['category_id',$child['id']]])->first();
