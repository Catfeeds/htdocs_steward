...
class MtFoodGoodsRequest implements GoodsFactoryInterface
{
    ...
    public function set_sort_category($args)
    {
        ...
        foreach ($args['o_category_id'] as $key=>$category_name) {
            if ($args['level'] == 1) {
                $request = [
                    'app_poi_code' => $mall_data->code,
                    'category_name_origin' => $category_name,
                ...
                if (!isset($args['pid']) || !ebsig_is_int($args['pid'])) {
                    return ['code' => 400, 'message' => '参数错误：pid'];
                }

                $category_data = StCategory::find($args['pid']);

                $request = [
                    'app_poi_code' => $mall_data->code,
                    'category_name_origin' => $category_data->name,
        ...
        return ['code' => 200, 'message' => '美团分类排序设置成功'];
    ...
    public function batch_update_stock($args_data)
    {
        ...
        return $this->curl->call($request_data,'retail/sku/stock','post');
    }
    ...
    public function batch_create_product ($args_data) {
        ...
        $num = ceil(count($food_data)/200);

        error_log('数量');
        error_log($num);

        $request = array_chunk($food_data, $num);
        ...
        try {
            ...
            foreach ($request as $item) {
                ...
                $args = [
                    'app_poi_code' => $mall_data->code,
                    'food_data' => json_encode($item)
                ];

                $insert_res =  $this->curl->call($args, 'retail/batchinitdata', 'post');
                ...
                error_log('返回数据');
                error_log(var_export($insert_res,true));
    ...
    public function update_product($args_data)
    {
        ...
        $request = [
            ...
            'name' => $goodsName,
            'price' => 0,
            'picture' => 'http://'.$_SERVER['HTTP_HOST'].$args_data['goods_image'],
            'category_name' => $category->name,
            'skus' => []
        ];
    ...
    public function get_shop_product($args_data)
    {
        ...
        $result = $this->curl->call($request,'retail/list');
    ...
    public function category_list () {
        ...
        $result = $this->curl->call($args_data, 'retailCat/list');
        error_log('======//分类列表');
