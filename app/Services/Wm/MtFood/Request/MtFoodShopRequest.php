...
class MtFoodShopRequest implements ShopFactoryInterface
{
    ...
    public function edit_shop($args_data)
    {
        ...
        if (!isset($args_data['shipping_fee']) || !ebsig_is_int($args_data['shipping_fee'],2)) {
        ...
        if (!isset($args_data['third_tag_name'])) {
        ...
        $request = [
            ...
            'shipping_time' => $args_data['shipping_time'],
            'open_level' => 1,
            'is_online' => 1,
            'third_tag_name' => '肉夹馍'
        ];
     ...
     * @return array
     */
    public function business_state ($args_data) {

        if (!isset($args_data['type']) || !ebsig_is_int($args_data['type'],2)) {
            return ['code' => 404, 'message' => '参数错误：type'];
        }

        if (!isset($args_data['mall_code']) || empty($args_data['mall_code'])) {
            return ['code' => 404, 'message' => '参数错误：mall_code'];
        }
        ...
        if ($args_data['type'] == 1) {
            return $this->curl->call($request, 'poi/open','post');
        }else{
            return $this->curl->call($request, 'poi/close','post');
        }
